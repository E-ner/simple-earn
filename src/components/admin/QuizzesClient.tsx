  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-black text-(--text-primary) tracking-tight">Quiz Library</h2>
        <button onClick={() => setShowQuizForm(!showQuizForm)} 
            className="flex items-center gap-2 px-4 py-2 bg-(--accent) text-white rounded-lg text-xs font-bold hover:opacity-90">
      ...
      {showQuizForm && (
        <div className="p-6 bg-(--surface) border border-(--border) rounded-xl space-y-4 shadow-xl">
          ...
          <button onClick={handleCreateQuiz} disabled={isPending || !quizForm.title || !quizForm.question} className="px-6 py-2 bg-(--accent) text-white text-xs font-bold rounded-md hover:opacity-90 disabled:opacity-50">Save Quiz to Database</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-4">
          ...
          <div key={q.id} className="p-4 bg-(--surface) border border-(--border) rounded-lg flex flex-col gap-3">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-[10px] text-(--text-tertiary) uppercase mt-1 font-bold">Reward: ${Number(q.reward || 0.50).toFixed(2)}</p>
              </div>
              <button onClick={() => handleDeleteQuiz(q.id)} disabled={isPending} className="p-2 text-(--error) bg-red-500/10 hover:bg-red-500/20 rounded transition-colors flex gap-2 items-center text-[10px] font-black uppercase"><Trash2 className="w-4 h-4" /> Delete</button>
            </div>
        ...
        {initialQuizzes.length === 0 && <div className="p-10 text-center text-(--text-tertiary) text-xs font-bold uppercase tracking-widest border border-dashed border-(--border) rounded-xl">No Quizzes Exist</div>}
      </div>
    </div>
  )
}
