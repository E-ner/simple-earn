...
export default function BalanceCards({ user, currency, dict }: BalanceCardsProps) {
  const balances = [
    ...
    { 
      ...
      color: 'from-[var(--purple)] to-[#7c3aed]',
    ...
    >
      {balances.map((card, i) => (
        <motion.div
          key={i}
          variants={item}
          className="relative group p-6 rounded-[var(--radius-xl)] bg-[var(--surface)] border border-[var(--border)] hover:border-[var(--border-hover)] transition-all overflow-hidden"
        >
          ...
          <div className="relative z-10 flex flex-col h-full">
            <div className="flex justify-between items-start mb-4">
              <div className={`p-2.5 rounded-md bg-gradient-to-br ${card.color} bg-opacity-10 flex items-center justify-center text-[var(--text-primary)]`}>
                {card.icon}
              </div>
              <span className="text-[10px] font-black tracking-widest uppercase text-[var(--text-tertiary)] group-hover:text-[var(--accent)] transition-colors">
                {card.badge}
              </span>
            </div>
            
            <div className="mt-auto">
              <p className="text-[10px] font-bold text-[var(--text-tertiary)] uppercase tracking-widest mb-1.5">{card.label}</p>
