...
function StatCard({ label, value, icon: Icon, color }: { label: string, value: string | number, icon: any, color: string }) {
  return (
    <div className="p-6 bg-(--surface) border border-(--border) rounded-xl">
      ...
      <div className="text-3xl font-black text-(--text-primary) tracking-tighter font-mono mb-1">{value}</div>
      <div className="text-[10px] text-(--text-tertiary) uppercase tracking-widest font-bold">{label}</div>
    </div>
  )
}
    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="mb-10">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[10px] font-black text-(--purple) uppercase tracking-widest">Root Administration</span>
        </div>
        <h1 className="text-3xl font-black text-(--text-primary) tracking-tighter">System Overview</h1>
        <p className="text-sm text-(--text-tertiary) mt-1">Full platform control and management dashboard.</p>
      </div>
                ...
                {pending.slice(0, 10).map((tx: any) => (
                  <tr key={tx.id} className="border-b border-(--border) hover:bg-(--surface-2) transition-colors">
                    <td className="px-4 py-3">
                      ...
                      <p className="text-(--text-tertiary)">{tx.user.email}</p>
                    </td>
                    <td className="px-4 py-3 font-mono font-bold text-(--text-secondary)">{tx.type}</td>
                    <td className="px-4 py-3 font-mono font-black text-(--accent)">${Number(tx.amount).toFixed(2)}</td>
                    <td className="px-4 py-3 text-(--text-tertiary)">{tx.paymentMethod || '—'}</td>
                    <td className="px-4 py-3 text-(--text-tertiary)">{new Date(tx.createdAt).toLocaleDateString()}</td>
