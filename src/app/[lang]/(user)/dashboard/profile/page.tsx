  ...
  return (
    <div className="flex flex-col gap-10 pb-20">
      <header className="mb-2 space-y-2">
        ...
        <p className="text-(--text-tertiary) max-w-2xl leading-relaxed uppercase font-black text-[10px] tracking-widest italic">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <motion.div
          ...
          className="p-8 rounded-md bg-(--surface) border border-(--border) space-y-8"
        >
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-sm bg-(--accent)/10 text-(--accent)">
              <User className="w-5 h-5" />
            </div>
            <h2 className="text-[11px] font-black uppercase tracking-[0.2em] text-(--text-primary)">Identity Parameters</h2>
          </div>

          <form onSubmit={handleUpdateProfile} className="space-y-6">
            <div className="space-y-2">
              <label className="text-[9px] font-black uppercase tracking-widest text-(--text-tertiary)">Authorized Username</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full h-11 bg-(--surface-2) border border-(--border) rounded-md px-4 text-sm text-(--text-primary) focus:border-(--accent) outline-none transition-colors"
                  placeholder="+1..."
                />
              </div>
              <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                className="w-full h-11 bg-[#16161f] border border-white/8 rounded-md px-4 text-sm text-white focus:border-(--accent) outline-none transition-colors"
              >
            ...
            <button
              ...
              className="w-full h-11 rounded-md bg-(--text-primary) text-(--bg-elevated) text-[10px] font-black uppercase tracking-widest hover:opacity-90 transition-opacity flex items-center justify-center gap-2"
            >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-[11px] font-black uppercase tracking-[0.2em] text-(--text-primary)">Security Key Management</h2>
          </div>
          ...
          <form onSubmit={handleUpdatePassword} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full h-11 bg-white/[0.03] border border-white/8 rounded-md px-4 text-sm text-white focus:border-(--accent) outline-none transition-colors"
              />
            </div>
      ...
      <motion.div
        ...
        className="p-8 rounded-md bg-(--surface) border border-red-500/10 space-y-6"
      >
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-sm bg-red-500/10 text-red-500">
          ...
          <div>
            ...
            <p className="text-[9px] text-(--text-tertiary) uppercase font-bold tracking-widest mt-1">High-Risk Operation: Irreversible network exit.</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6 p-6 rounded-md bg-red-500/[0.03] border border-red-500/10">
          <div className="space-y-1">
            <h3 className="text-xs font-bold text-(--text-primary) uppercase italic">Data Purge Warning</h3>
        ...
        {isDeleteModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <motion.div
              ...
              className="w-full max-w-md bg-(--bg-elevated) border border-(--border) p-8 rounded-md space-y-6 shadow-2xl shadow-red-500/10"
            >
              <div className="flex flex-col items-center text-center gap-4">
                ...
                <div className="space-y-2">
                  <h3 className="text-sm font-black uppercase tracking-widest text-(--text-primary)">Confirm Data Purge?</h3>
                  <p className="text-[10px] text-(--text-secondary) uppercase tracking-wide">Enter the institutional code below to confirm account termination.</p>
                </div>
              </div>

              <div className="space-y-2">
                <p className="text-center text-[9px] font-mono text-(--text-tertiary) uppercase">Verification Code: <span className="text-(--text-primary) font-bold select-all">TERMINATE-PURGE-DATA</span></p>
                <input
                  type="text"
                  placeholder="Type verification code..."
                  id="termination-confirm"
                  className="w-full h-11 bg-(--surface-2) border border-(--border) rounded-md px-4 text-center text-sm text-(--text-primary) focus:border-red-500 outline-none transition-colors"
                  onChange={(e) => {
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 h-11 rounded-md bg-white/[0.03] border border-white/8 text-[10px] font-black uppercase tracking-widest text-white hover:bg-white/[0.06] transition-colors"
                >
                  Abort Protocol
                </button>
                <button
